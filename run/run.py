    ...
    with open(src_filename) as f1, open(trg_filename) as f2:
        for line1, line2 in zip(f1, f2):
            examples.append(
                Example(
                    idx = idx,
                    source = json.loads(line1.strip())["INPUT"],
                    target = json.loads(line2.strip())["INPUT"],
                )
            )
            idx += 1
    return examples
...
def main():
    ...
    parser.add_argument("--output_size", default=10, type=int,
                        help="output size for beam search")
            ...
            if args.do_eval and (int(epoch) % 2 == 0 or int(epoch) > int(args.num_train_epochs) - 5 or int(epoch) <= 5 ):
                ...
                with open(os.path.join(args.output_dir,"dev.output"),'w') as f, open(os.path.join(args.output_dir,"dev.gold"),'w') as f1:
                    for ref,gold in zip(p,eval_examples):
                        ...
                        f.write(repr(ref)[1:-1]+'\n')
                        f1.write(repr(gold.target)[1:-1]+'\n')     
        ...
        for idx, file in enumerate(files):
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                ...
                with torch.no_grad():
                    ...
                    for pred in preds:
                        inner_p = []
                        for out_index in range(args.output_size):
                            t = pred[out_index].cpu().numpy()
                            t = list(t)
                            if 0 in t:
                                t = t[:t.index(0)]
                            text = tokenizer.decode(t, clean_up_tokenization_spaces=False)
                            inner_p.append(text.strip())
                        p.append(inner_p) 
            model.train()
            predictions = []
            accs = []
            with open(os.path.join(args.output_dir, "test_{}_{}.output".format(str(test_file_name), args.output_size)), 'w') as f, open(
                    os.path.join(args.output_dir, "test_{}_{}.gold".format(str(test_file_name), args.output_size)), 'w') as f1:
                for ref, gold in zip(p, eval_examples):
                    # predictions.append(str(gold.idx) + '\t' + ref)
                    f1.write(repr(gold.target)[1:-1] + '\n')
                    correct_flag = False
                    for beam_pred in ref:
                        f.write(repr(beam_pred)[1:-1] + '\n')
                        if beam_pred == gold.target:
                            correct_flag = True
                    if correct_flag:
                        accs.append(1)
                    else:
